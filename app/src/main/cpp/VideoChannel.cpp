...
void VideoChannel::encode(uint8_t *data) {
    ...
    pic_in.img.plane[0] = data + ySize + uSize;

    //TODO 编码的i_pst 每次需要增长
    pic_in.i_pts = i_pts++;

    x264_picture_t pic_out;
    x264_nal_t *pp_nal;
    int pi_nal;
    //pi_nal：输出了多少nal ---》pp_nal[] 有多少个元素
    ...
    for (int i = 0; i < pi_nal; ++i) {
        ...
        //00 00 00 01-》 间隔 sps pps IDR,真正的数据是不需要分隔符
        if (type == NAL_SPS) {
            //sps 后面肯定跟着pps
            spslen = i_payload - 4;// 去掉间隔00 00 00 01
            sps = (uint8_t *) alloca(spslen);//栈中申请内存，不需要释放
            memcpy(sps, p_payload + 4, spslen);// 便宜间隔

        } else if (type == NAL_PPS) {
            ppslen = i_payload - 4;// 去掉间隔00 00 00 01
            pps = (uint8_t *) alloca(ppslen);//栈中申请内存，不需要释放
            memcpy(pps, p_payload + 4, ppslen);// 便宜间隔

            //pps 后面肯定有I帧，发送I帧之前一定先发出sps与pps
...
void VideoChannel::sendVideoConfig(uint8_t *sps, uint8_t *pps, int spslen, int ppslen) {
    int bodySize = 13 + spslen + 3 + ppslen;// 固定的格式
